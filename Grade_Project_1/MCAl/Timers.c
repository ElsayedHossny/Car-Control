...
static void (*Timer0_OCF_Fptr) (void)=NULLPTR;
/*Timer 1*/
...
u32 TIME_OF_FUN(u16 t1_B , u16 t2_A ,u8 C_IN){
	//TimeOut=TimeFun + Response
	u32 NOfTick;
	NOfTick = (C_IN*256)+ (t2_A - t1_B);
	return (NOfTick*Tick_time);
}
...
void Timer1_Init( Timer1Mode_type mode,Timer1Scaler_type scaler)
{
	switch (mode)
	{
		case TIMER1_PhaseCorrect_ICR1_MODE:
